...
const modelCallLogSchema = z.object({
  ...
  anchorType: z.enum(["character", "scene"]).optional(),
  ...
  requestHost: z.string().max(253).optional(),
  requestPath: z.string().max(200).optional(),
  region: z.string().max(80).optional(),
  workspaceIdMasked: z.string().max(80).optional(),
  apiMode: z.string().max(40).optional(),
  payloadBytes: z.number().int().nonnegative().optional(),
  submissionTimeoutMs: z.number().int().positive().optional(),
  referenceSourceTypes: z.array(z.string().max(40)).max(3).optional(),
  failurePhase: z.string().max(40).optional(),
  networkErrorName: z.string().max(80).optional(),
  networkErrorMessage: z.string().max(240).optional(),
  networkCauseCode: z.string().max(80).optional(),
  networkCauseErrno: z.number().int().optional(),
  networkCauseSyscall: z.string().max(80).optional(),
  errorName: z.string().max(80).optional(),
  causeCode: z.string().max(80).optional(),
  requestStartedAt: z.number().int().nonnegative().optional(),
  requestCompletedAt: z.number().int().nonnegative().optional(),
...
function sanitizeLog(entry: ModelCallLog): ModelCallLog {
  ...
  return modelCallLogSchema.parse({
    ...
    ...(entry.networkErrorMessage ? { networkErrorMessage: clean(entry.networkErrorMessage, 240) } : {}),
