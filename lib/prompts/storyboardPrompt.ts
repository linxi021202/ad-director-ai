...
import { CHARACTER_SCENE_STATE_FIELDS, SCENE_STATE_FIELDS, STORYBOARD_SCHEMA_VERSION, STORYBOARD_SHOT_OUTPUT_FIELDS, storyboardContractExample } from "../ai/contracts/storyboard";
...
export function buildStoryboardChunkPrompt(brief: ProductBrief, strategy: AdStrategy, input: StoryboardChunkInput): string {
  ...
  const previousContext = input.previousShot
    ...
    : "这是第一段，不需要继承更早镜头。";
  const exampleShot = storyboardContractExample(firstIndex, input.shotDurationPlan[0]);
...
正式分镜契约版本：${STORYBOARD_SCHEMA_VERSION}。镜头只允许这些字段：${STORYBOARD_SHOT_OUTPUT_FIELDS.join("、")}。
sceneStateBefore / sceneStateAfter 只允许这些字段：${SCENE_STATE_FIELDS.join("、")}。
characterStates 只允许这些字段：${CHARACTER_SCENE_STATE_FIELDS.join("、")}，不得创造 mentalState、internalFeeling、actorMood、bodyEmotion 等字段。
emotion 表示内部整体情绪；expression 表示画面可见表情；gaze 表示视线目标；handState 表示双手状态，四者不可混用。
