...
export const DEFAULT_HERO_SHOT_ID = "shot-3";
...
export function resolveHeroShot(
  ...
  heroShotId: string = DEFAULT_HERO_SHOT_ID
) {
  return shots.find((shot) => shot.id === heroShotId)
    ?? shots.find((shot) => shot.index === 3)
    ?? shots[0]
    ?? null;
}
...
export function buildOptimizedVideoPrompt(shot: StoryboardShot) {
  return appendNoReadableTextRules([
    "使用用户上传的真实产品图作为产品外观参考，并使用当前主镜头关键帧作为场景构图参考，生成 4-5 秒广告视频。",
