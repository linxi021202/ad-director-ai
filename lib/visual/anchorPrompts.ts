...
export function buildCharacterCandidatePrompt(brief: CharacterAnchorBrief, direction: CharacterCandidateDirection) {
  return appendNoReadableTextRules(appendSingleFrameConstraint([
    ...
    `选角定位：${direction.castingPositioning}；年龄质感：${direction.ageTexture}。`,
    `脸型骨相：${direction.faceStructure}；五官特征：${direction.facialFeatures}；发型处理：${direction.hairstyle}。`,
    `服装气质：${direction.wardrobeMood}；身体语言：${direction.bodyLanguage}。`,
