...
describe("homepage final hero", () => {
  ...
  it("includes responsive navigation and reduced-motion support", () => {
    ...
    expect(transition).toContain("const count = width < 768 ? 175 : 340");
    expect(transition).toContain("Math.min(window.devicePixelRatio || 1, 1.5)");
    expect(transition).toContain("particle.depth");
    expect(transition).toContain("window.cancelAnimationFrame(frameId)");
    expect(transition).toContain('window.removeEventListener("resize", resize)');
    expect(page).toContain("1660");
    expect(page).toContain("2450");
    ...
    expect(styles).toContain("transition-space-drift");
    expect(styles).toContain("transition-aperture");
    expect(transition).toContain("drawCore(context");
    expect(transition).not.toContain("home-route-transition__ring");
  });
