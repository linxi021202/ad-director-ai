...
export async function GET(_request: Request, context: RouteContext) {
  ...
  const capability = await getSessionWanCapability(session.id);
...
export async function DELETE(_request: Request, context: RouteContext) {
  ...
  return response(
    true,
    { deleted: true, capability: await getSessionWanCapability(session.id) },
    null,
    200
  );
}

async function getSessionWanCapability(sessionId: string) {
  const config = getAIConfig({ allowSessionSecrets: true });
  const secret = await resolveProviderSecret("wan", sessionId);
  return getWanVideoCapability(true, config.realVideoEnabled && Boolean(secret.value)).capability;
}
