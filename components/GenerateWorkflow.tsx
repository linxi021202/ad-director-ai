          ...
          <section className="generation-stage-v3 stage-canvas workspace-column workspace-column--main">
            ...
            {storyboardPromptsIncomplete ? <section className="shot-prompt-status" aria-label="详细提示词任务状态">
              ...
              <div className="shot-prompt-status__list">{activeProject.shots.map((shot) => {
                ...
                return <div className="shot-prompt-status__row" key={shot.id}><span>镜头 {String(shot.index).padStart(2, "0")}</span><span className={status === "failed" ? "is-error" : ""}>{label}</span><span>{status === "completed" ? "图片与视频提示词已保存" : status === "failed" ? "可单独重试；详情见调用日志" : "等待生成"}</span><span className="shot-prompt-status__row-actions">{status === "failed" ? <CallLogDrawer projectId={activeProject.id} projectName={activeProject.brief.productName} label="日志" focusShotId={shot.id} /> : null}{status !== "completed" ? <button type="button" disabled={stageLocking} onClick={() => void retryStoryboardPromptExpansion(shot.id)}>{status === "failed" ? "重试" : "生成"}</button> : null}</span></div>;
              })}</div>
            </section> : null}
