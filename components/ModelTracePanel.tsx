...
export function ModelTracePanel({ status, traces = [], compact = false }: ModelTracePanelProps) {
  ...
  const rows: ModelInvocationTrace[] = [
    ...
    { id: "happyhorse", title: "HappyHorse 视频", stage: "多参考图生成", provider: "百炼", model: status.videoModel || "happyhorse-1.0-r2v", fallbackUsed: false },
    ...
    <section className="rounded-2xl border border-line bg-panel p-5 md:p-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          {!compact ? <p className="mt-3 max-w-[62ch] text-sm leading-6 text-muted">当前主链路为 DeepSeek 文本、Qwen-Image 关键帧、HappyHorse 主镜头、Remotion 计划合成；HappyHorse 默认通过百炼生成主镜头视频，手动导入仅作为接口不可用时的备用方式。</p> : null}
        </div>
